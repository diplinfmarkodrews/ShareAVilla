...
namespace ShareAVilla.Controllers
{
    ...
    public class AccountController : Controller
    {
        ...
        public async Task<ActionResult> Login(LoginViewModel model, string returnUrl)
        {
            ...
            switch (result)
            {
                ...
                case SignInStatus.Success:
                    ...
                    {
                        return RedirectToAction("Index", "VillaProfiles");
                    }
                    else
                    {
                        if (user.ProfileID != -1)
        ...
        public async Task<ActionResult> Register(RegisterViewModel model)
        {
            if (ModelState.IsValid)
            {
                var user = new ApplicationUser { UserName = model.Email, Email = model.Email, UserType = model.UserType, ProfileID = -1 };
                ...
                if (result.Succeeded)
                {
                    ...
                    if (string.Compare(model.UserType, "Villa") == 0)
                    {
                        return RedirectToAction("Create", "VillaProfiles");
                    }
